    ...
    def run_analysis():
        ...
        if resume_from_stage in ["stock_analysis_complete", "ratio_analysis_complete"]:
            ...
            if checkpoint_stage == "stock_analysis" and cached_data and "sectors" in cached_data:
                print(f"  🔄 Resuming stock_analysis - loading already processed results...", flush=True)
                
                # CRITICAL: Use Supabase as source of truth for which stocks are actually z-scored
                try:
                    date_str = datetime.now().strftime("%Y-%m-%d")
                    ...
                    for sector in output_sectors:
                        for stock in sector.get("stocks", []):
                            ...
                            processed_tickers_from_checkpoint.append(ticker)
                    print(f"  ✓ Loaded {len(results)} already processed stocks from checkpoint (fallback)", flush=True)
                ...
                for ticker in sorted(tickers):
                    ...
                    if ticker in processed_tickers_from_checkpoint:
                        ...
                        if z_data:
                            ...
                            continue  # Skip processing if z-score found
                        else:
                            # Checkpoint says processed but no z-score in Supabase - process it anyway
                            print(f"  ⚠ {ticker} marked as processed in checkpoint but no z-score in Supabase - will process", flush=True)
                            # Don't continue - let it fall through to processing
                    ...
                    for attempt in range(1, max_retries + 1):
                        try:
                            ...
                            with ThreadPoolExecutor(max_workers=1) as executor:
                                ...
                                timeout_detected = threading.Event()  # Flag to signal timeout to main thread
                                    ...
                                    while not progress_stop.is_set():
                                        ...
                                        if not progress_stop.is_set():
                                            ...
                                            if stock_elapsed < timeout_seconds:
                                                ...
                                                if check_timeout_and_save_checkpoint("stock_analysis", {"sectors": []}, processed_tickers):
                                                    progress_stop.set()
                                                    timeout_detected.set()  # Signal main thread to stop
                                                    # Prepare partial response
                                                    partial_response = {
                                                        "sectors": [],
                                                    ...
                                                    partial_response["sectors"] = list(sector_map.values())
                                                    save_cache(partial_response, is_partial=True, stage="stock_analysis", processed_tickers=processed_tickers)
                                                    return  # Exit monitor thread, main thread will handle exception
                                            else:
                                                break
                                ...
                                try:
                                    # Check for timeout periodically while waiting for result
                                    result = None
                                    check_interval = 2  # Check every 2 seconds
                                    elapsed_wait = 0
                                    while elapsed_wait < timeout_seconds:
                                        if timeout_detected.is_set():
                                            progress_stop.set()
                                            try:
                                                future.cancel()
                                            except:
                                    ...
                                    if result is None and not timeout_detected.is_set():
                                        # Timeout reached but no timeout checkpoint detected
                                        progress_stop.set()
                                        try:
                                            future.cancel()
                                        except:
                                            pass
                                        raise FutureTimeoutError(f"Stock analysis timed out after {timeout_seconds}s")
                                except Exception as e:
                            ...
                            try:
                                ...
                                with ThreadPoolExecutor(max_workers=1) as executor:
                                    ...
                                    try:
                                        ...
                                        if ratio_score is None and not timeout_detected.is_set():
                                            # Timeout reached but no timeout checkpoint detected
                                            progress_stop.set()
                                            try:
                                                future.cancel()
                                                ...
                                                pass
                                            raise FutureTimeoutError(f"Ratio calculation timed out after {timeout_seconds}s")
                                    except Exception as e:
                                        if "TIMEOUT_CHECKPOINT" in str(e):
                                            raise  # Re-raise timeout checkpoint exception
                                        elif isinstance(e, FutureTimeoutError):
